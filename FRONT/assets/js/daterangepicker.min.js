...
!(function (t, e) {
    ...
    var a = function (a, i, s) {
        ...
        if ('object' == typeof i.ranges) {
            for (l in i.ranges) {
                ...
                var m = this.maxDate
                if (
                    (this.maxSpan && m && o.clone().add(this.maxSpan).isAfter(m) && (m = o.clone().add(this.maxSpan)), m && h.isAfter(m) && (h = m.clone()), !((this.minDate && h.isBefore(this.minDate, this.timepicker ? 'minute' : 'day')) || (m && o.isAfter(m, this.timepicker ? 'minute' : 'day'))))
                ) {
                    var p
        ...
        'function' == typeof s && (this.callback = s),
            ...
            this.singleDatePicker &&
                (this.container.addClass('single'), this.container.find('.drp-calendar.left').addClass('single'), this.container.find('.drp-calendar.left').show(), this.container.find('.drp-calendar.right').hide(), !this.timePicker && this.autoApply && this.container.addClass('auto-apply')),
            ...
            this.element.is('input') || this.element.is('button')
                ? this.element.on({ 'click.daterangepicker': e.proxy(this.show, this), 'focus.daterangepicker': e.proxy(this.show, this), 'keyup.daterangepicker': e.proxy(this.elementChanged, this), 'keydown.daterangepicker': e.proxy(this.keydown, this) })
    ...
    return (
        (a.prototype = {
            ...
            updateView: function () {
                this.timePicker &&
                    (this.renderTimePicker('left'), this.renderTimePicker('right'), this.endDate ? this.container.find('.right .calendar-time select').prop('disabled', !1).removeClass('disabled') : this.container.find('.right .calendar-time select').prop('disabled', !0).addClass('disabled')),
            ...
            renderCalendar: function (a) {
                ...
                var v = 'left' == a ? this.minDate : this.startDate,
                    C = this.maxDate,
                    Y = ('left' == a ? this.startDate : this.endDate, this.locale.direction, '<table class="table-condensed"><caption>이전달, 연도/월, 다음달, 일, 월, 화, 수, 목, 금, 토 항목으로 구성된 달력 표</caption>')
                ;;;(Y += '<thead>'),
                    (Y += "<tr class='controlArea'>"),
                    (this.showWeekNumbers || this.showISOWeekNumbers) && (Y += '<th scope="col"></th>'),
                    (v && !v.isBefore(i.firstDay)) || (this.linkedCalendars && 'left' != a) ? (Y += '<th></th>') : (Y += '<th scope="col" class="prev available"><button type="button"><em class="hide">이전</em></button></th>')
                ...
                if (this.showDropdowns) {
                    ...
                    A += '</select>'
                    for (var N = '<select class="yearselect">', E = I; E <= M; E++) N += '<option value="' + E + '"' + (E === x ? ' selected="selected"' : '') + '>' + E + '년</option>'
                    w = (N += '</select>') + A 
                }
                if (
                    ((Y += '<th scope="col" colspan="5" class="month">' + w + '</th>'),
                    (C && !C.isAfter(i.lastDay)) || (this.linkedCalendars && 'right' != a && !this.singleDatePicker)
                        ? (Y += '<th scope="col" class="next available"><button type="button"><em class="hide">다음</em></button></th>')
                        : (Y += '<th scope="col" class="next available"><button type="button"><em class="hide">다음</em></button></th>'),
                    (Y += '</tr>'),
                    (Y += '<tr>'),
                    (this.showWeekNumbers || this.showISOWeekNumbers) && (Y += '<th scope="col" class="week">' + this.locale.weekLabel + '</th>'),
